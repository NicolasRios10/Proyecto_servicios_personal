...
public class ProfesorCursoServiceImpl implements ProfesorCursoService {
	...
	private boolean checkUnionExistsProfesorCurso(ProfesorCursoDto profesorCursoDto) {
		List<ProfesorCursoEntity> listProfesorCursoEntity = profesorCursoRepository.findAll();

		for (ProfesorCursoEntity entity : listProfesorCursoEntity) {
			if (entity.getIdProfesor() == profesorCursoDto.getIdProfesor() && entity.getIdCurso() == profesorCursoDto.getIdCurso()) {
				return true;
			}
		}
	...
	public ResponseDto createProfesorCurso(ProfesorCursoDto profesorCursoDto) {
		try {
			boolean unionExiste = checkUnionExistsProfesorCurso(profesorCursoDto);
			if (unionExiste) {
	...
	@Override
	public ResponseDto deleteProfesorCurso(Long id) {
		try {
			ProfesorCursoEntity profesorCursoEntity = profesorCursoRepository.findById(id).orElse(null);
			if (profesorCursoEntity != null) {
				profesorCursoRepository.delete(profesorCursoEntity);
